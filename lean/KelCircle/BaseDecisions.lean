...
def fullGate {γ : Type} (s : CircleState) (signer : MemberId)
  ...
  baseGate s signer sequencerId d && appGate appState d

-- If base gate rejects, full gate rejects regardless of app gate
theorem full_gate_base_rejects {γ : Type} (s : CircleState)
    (signer : MemberId) (sid : MemberId) (d : BaseDecision)
    (appState : γ) (appGate : γ → BaseDecision → Bool)
    (hbase : baseGate s signer sid d = false) :
    fullGate s signer sid d appState appGate = false := by
  simp [fullGate, hbase]

-- If app gate rejects, full gate rejects regardless of base gate
theorem full_gate_app_rejects {γ : Type} (s : CircleState)
    (signer : MemberId) (sid : MemberId) (d : BaseDecision)
    (appState : γ) (appGate : γ → BaseDecision → Bool)
    (happ : appGate appState d = false) :
    fullGate s signer sid d appState appGate = false := by
  simp [fullGate, happ]
